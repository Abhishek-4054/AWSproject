...
# user_clusters.csv is automatically uploaded to S3 by train_kmeans.py after every run.
...
print('Model artifacts extracted to ./models/kmeans_artifacts/')

# ── Verify user_clusters.csv made it to S3 ────────────────────────────────────
S3_CLUSTERS_KEY = 'models/kmeans_artifacts/user_clusters.csv'
try:
    s3.head_object(Bucket=BUCKET, Key=S3_CLUSTERS_KEY)
    print(f'✅ user_clusters.csv confirmed in S3: s3://{BUCKET}/{S3_CLUSTERS_KEY}')
except Exception:
    print('⚠️  user_clusters.csv NOT found in S3 — uploading now...')
    s3.upload_file('./models/kmeans_artifacts/user_clusters.csv', BUCKET, S3_CLUSTERS_KEY)
    print(f'✅ Uploaded to s3://{BUCKET}/{S3_CLUSTERS_KEY}')
